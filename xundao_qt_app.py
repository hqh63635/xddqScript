...
class XundaoWindow(FramelessWindow):
    ...
    def _footer(self) -> QFrame:
        ...
        layout = QHBoxLayout(footer); set_margins(layout, 20, 0, 20, 0); layout.setSpacing(12)
        self.start_button = PrimaryPushButton(FIF.PLAY, "启动脚本"); self.start_button.clicked.connect(self.start_chop)
        self.stop_button = PushButton(FIF.CLOSE.icon(QColor("#d94f4f")), "停止")
        self.stop_button.clicked.connect(self.stop_chop)
        self.start_button.setFixedSize(136, 36)
        self.stop_button.setFixedSize(96, 36)
